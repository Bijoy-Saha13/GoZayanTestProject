...
|---|---|---|---|
| **FR-01** | **Route header** shows: origin → destination (IATA codes), the date, passenger count, the text **"One Way"**, and an **Edit** button. Edit may be decorative (no action). | MUST `[PDF p.2]` | Visual check against Figma; values come from `FlightSearchRequest`, not from the API response. |
...
| FR-07 | Carousel is placed after the **2nd** flight card. If there are fewer than 2 offers, it is placed after the last card. Shown only in the success state. | SHOULD `[DECISION D-06]` | 1 offer → carousel after card 1; 5 offers → after card 2; empty/error → no carousel. |
...
|---|---|---|---|---|
| **ST-01** | `loading` | Initial state; and on every (re)load / retry. | Header + date strip visible. Content area shows N (= 4) skeleton cards with shimmer. Sort control disabled. No carousel. | MUST |
...
| MAP-07 | **Airline** = first leg's `airline`. If legs have more than one distinct airline, show `"<first airline> & more"` unless Figma shows a different treatment. Logo = itinerary `airline_logo`, else first leg's. | SHOULD `[DECISION D-09]` `[FIGMA]` |
...
| MAP-12 | **Next-day arrival**: if arrival calendar date > departure calendar date, expose `dayOffset` (e.g. `+1`) for the card, if the design shows it. | SHOULD `[FIGMA]` |
...
| FMT-04 | Header date | `EEE, dd MMM` | `Wed, 14 Oct` `[FIGMA]` |
| FMT-05 | Strip chip date | `EEE dd MMM` | `Sun 08 Feb` `[PDF p.2]` |
| FMT-06 | Passengers | `1 Traveller` / `n Travellers` | `[FIGMA]` |
...
| UI-09 | **All four states can be demonstrated on demand** in DEBUG via launch argument `-FlightsStubState loading|success|empty|error` (swaps in a stub service; `loading` never completes). Needed because a cached response loads instantly and reviewers must "see all of them". | SHOULD `[PDF p.2]` `[DECISION D-18]` |
...
| ID | Requirement | Priority |
|---|---|---|
| EXT-01 | The **"Cheapest" dropdown** works (`UIButton` + `UIMenu`) with two options: **Cheapest** (lowest price) and **Fastest** (shortest total duration). Button title reflects the current option. | MAY |
...
| A-11 | `ViewController.swift`, `HomeScreen/*` | Leftover template `ViewController`; "Home" naming for a Flight Results screen; empty XIB. | Dead code / misleading names. | Delete `ViewController.swift`; rename to `FlightResults…`; decide XIB vs code (D-14). |
...
| D-09 ❓ | Multi-airline itineraries | First leg's airline name (+ "& more" if multiple), per Figma if it shows otherwise. | Card has one airline slot. |
...
| D-14 ❓ | UI framework / layout | UIKit, programmatic layout (remove the empty XIB). | Project already UIKit + programmatic window; programmatic cells diff cleanly in reviews. |
...
## 13. Figma — items still to capture ⚠️

The Figma link in the brief (`figma.com/design/mMLQa70hIjiWeNvRzgNhQS/...`) **requires login and could not be read** while writing this spec. Frames referenced: **"Flight Result, loading"** and **"Flight Result, with results"**. Before building UI, capture and add here:

- [ ] Colours (background, primary/brand, text primary/secondary, chip selected/unselected, card, divider, shimmer base/highlight)
- [ ] Typography (font family, sizes/weights for route, times, codes, price, labels)
- [ ] Spacing, card corner radius, shadows
- [ ] Route header layout (back arrow? Edit button style), passenger/"One Way" formatting
- [ ] Date chip size, selected style, price style
- [ ] Sort control position and style ("Cheapest ▾")
- [ ] Flight card layout: airline logo/name placement, time/code/duration/stops arrangement, dotted route line, price label ("Starting from"?), "+1" day indicator
- [ ] Promo card size, image aspect ratio, title lines, Learn more style
- [ ] Skeleton card shape and count
- [ ] Time format (12 h vs 24 h), date format
- [ ] Icons/assets to export

Empty and error states are **not** in Figma → designed by us (D-05), consistent with the above tokens.
